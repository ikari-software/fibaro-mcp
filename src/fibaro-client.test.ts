...
describe('FibaroClient caching', () => {
  ...
  it('caches getDevices within TTL', async () => {
    const scope = nock('http://fibaro.test:80')
      .get('/api/devices')
      .once()
      .reply(200, [{ id: 1, name: 'D1', roomID: 1, type: 't', baseType: 'b', enabled: true, visible: true }]);
  ...
  it('invalidates rooms/devices list after room create/update/delete', async () => {
    ...
    const rooms3: any[] = [];
    const devices1 = [{ id: 1, name: 'D1', roomID: 1, type: 't', baseType: 'b', enabled: true, visible: true }];
    const devices2 = [{ id: 1, name: 'D1b', roomID: 1, type: 't', baseType: 'b', enabled: true, visible: true }];
    const devices3: any[] = [];
  ...
  it('invalidates sections/rooms/devices list after section create/delete', async () => {
    ...
    const sections2 = [{ id: 10, name: 'S1' }, { id: 11, name: 'S2' }];
    const sections3 = [{ id: 10, name: 'S1' }];
    const rooms1 = [{ id: 1, name: 'R1', sectionID: 10, visible: true, isDefault: false }];
    const rooms2 = [{ id: 1, name: 'R1', sectionID: 10, visible: true, isDefault: false }];
    const devices1 = [{ id: 1, name: 'D1', roomID: 1, type: 't', baseType: 'b', enabled: true, visible: true }];
    const devices2 = [{ id: 1, name: 'D1', roomID: 1, type: 't', baseType: 'b', enabled: true, visible: true }];
  ...
  it('invalidates devices list after setDeviceProperty', async () => {
    const devices1 = [{ id: 1, name: 'D1', roomID: 1, type: 't', baseType: 'b', enabled: true, visible: true }];
    const devices2 = [{ id: 1, name: 'D1c', roomID: 1, type: 't', baseType: 'b', enabled: true, visible: true }];
  ...
  it('user CRUD', async () => {
    ...
    expect(await client.updateUser(10, { email: 'n@example.com' })).toEqual({ id: 10, email: 'n@example.com' });
  ...
  it('geofence CRUD', async () => {
    ...
    expect(await client.createGeofence({ name: 'G2', latitude: 1, longitude: 2, radius: 3 })).toEqual({ id: 2, name: 'G2' });
  ...
  it('getRoom and getSection', async () => {
    ...
    const client = makeClient();
    expect(await client.getRoom(1)).toEqual({ id: 1, name: 'R', sectionID: 10, visible: true, isDefault: false });
  ...
  it('getDeviceStats with params', async () => {
    ...
    const client = makeClient();
    expect(await client.getDeviceStats(1, { from: 1, to: 2, property: 'power' })).toEqual({ ok: true });
    expect(scope.isDone()).toBe(true);
  });
...
describe('FibaroClient API methods', () => {
  ...
  it('scene lifecycle and actions', async () => {
    const scope = nock('http://fibaro.test:80')
      ...
      .once()
      .reply(200, { id: 1, name: 'S1', roomID: 10, type: 'com.fibaro.luaScene', isLua: true, visible: true, lua: 'print("hi")' })
  ...
  it('createScene defaults and getSceneLua fallback', async () => {
    const scope = nock('http://fibaro.test:80')
      .post('/api/scenes', { name: 'S1', roomID: 10, type: 'com.fibaro.luaScene', isLua: true, lua: '' })
      .once()
      .reply(200, { id: 2, name: 'S1', roomID: 10, type: 'com.fibaro.luaScene', isLua: true, visible: true })
      .get('/api/scenes/2')
      .once()
      .reply(200, { id: 2, name: 'S1', roomID: 10, type: 'com.fibaro.luaScene', isLua: true, visible: true });
  ...
  it('global variables CRUD', async () => {
    ...
    expect(await client.createGlobalVariable({ name: 'y', value: '9' })).toEqual({ name: 'y', value: '9' });
  ...
  it('quick app helpers filter devices and fetch lua payload', async () => {
    ...
    const scope = nock('http://fibaro.test:80')
      ...
      .once()
      .reply(200, [
        { id: 1, type: 'com.fibaro.binarySwitch', name: 'S', roomID: 1, baseType: 'b', enabled: true, visible: true },
        { id: 2, type: 'com.fibaro.QuickApp', name: 'QA', roomID: 1, baseType: 'b', enabled: true, visible: true },
      ])
      .get('/api/devices/2')
      .once()
      .reply(200, { id: 2, name: 'QA', properties: { code: 'print(1)', quickAppVariables: [{ name: 'x', value: 1 }] } });
